...
const ResizableTitle = (props: any) => {
  ...
  const menu = (
    <Menu>
      <Menu.Item key="delete" danger icon={<DeleteOutlined />} onClick={onDelete}>删除列</Menu.Item>
    </Menu>
  );
  const cellContent = (
    <Dropdown overlay={menu} trigger={['contextMenu']}>
      <div className="header-context-wrapper" style={{ height: '100%' }}>{restProps.children}</div>
    </Dropdown>
  );
...
const ResizableBodyCell = (props: any) => {
  ...
  const menu = (
    <Menu>
      <Menu.Item key="delete" danger icon={<DeleteOutlined />} onClick={onDelete}>删除行</Menu.Item>
    </Menu>
  );
  const cellContent = (
    <Dropdown overlay={menu} trigger={['contextMenu']}>
      <div className="body-context-wrapper" style={{ height: '100%', minHeight: '32px' }}>{children}</div>
    </Dropdown>
  );
  if (onRowResize) {
    return (
...
const DEFAULT_COLUMNS: TableColumn[] = [
  { title: '业务流程', dataIndex: 'business_process', key: 'business_process', width: 200, fixed: 'left' as const },
  { title: '数据统计', dataIndex: 'data_statistics', key: 'data_statistics', width: 200 },
  { title: '核心问题', dataIndex: 'core_issue', key: 'core_issue', width: 200 },
  { title: '关键原因', dataIndex: 'key_reason', key: 'key_reason', width: 200 },
  { title: '解决对策', dataIndex: 'solution', key: 'solution', width: 200 },
  { title: 'AI赋能点', dataIndex: 'ai_enablement', key: 'ai_enablement', width: 200 },
];

const DEFAULT_DATA: TableDataRow[] = Array.from({ length: 5 }).map((_, idx) => ({
  key: `row-${idx}`, business_process: '', data_statistics: '', core_issue: '', key_reason: '', solution: '', ai_enablement: '',
}));


/**
 * BusinessProcessTable - A table component for department business process analysis
 * Supports editable cells, resizable columns/rows, and auto-save functionality
 */
const BusinessProcessTable: React.FC<BusinessProcessTableProps> = memo(function BusinessProcessTable({ fontSize = 14 }) {
  ...
  const handleResize = (index: number) => (e: any, { size }: any) => {
    setColumns((prev) => { const next = [...prev]; next[index] = { ...next[index], width: size.width }; return next; });
  };
  ...
  if (isLoading) {
    return (
      <Card title={<span style={{ fontSize: '16px' }}>各部门业务流程分析 (Department Business Process Analysis)</span>} bordered={false} className="market-selection-card" styles={{ body: { padding: 0 } }}>
    ...
    <Card
      title={
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '16px' }}>各部门业务流程分析 (Department Business Process Analysis)</span>
